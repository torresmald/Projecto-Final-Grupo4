...
export class LoginComponent implements OnInit {
  ...
  public userLogin() {
    ...
    if (this.loginForm?.valid) {
      ...
      const request = this.teacher ? this.serviceTeacher.loginTeacher(this.loginForm?.value) 
      : this.serviceParents.loginParent(this.loginForm?.value);

     request.subscribe(() => {

      this.teacher ? alert('Te has logeado como maestro'): alert('Te has logeado como tutor')
